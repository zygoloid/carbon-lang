...
static auto ConvertSelf(Context& context, SemIR::LocId call_loc_id,
                        ...
                        SemIR::GenericInstanceId callee_instance_id,
  ...
  return ConvertToValueOfType(
      ...
      SemIR::GetTypeInstance(context.sem_ir(), callee_instance_id,
                             self_param.type_id));
}
  ...
  for (auto implicit_param_id : implicit_param_refs) {
    ...
    if (param.name_id == SemIR::NameId::SelfValue) {
      auto converted_self_id =
          ConvertSelf(context, call_loc_id, callee_id, callee_instance_id,
